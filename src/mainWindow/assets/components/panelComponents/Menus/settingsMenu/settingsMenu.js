function toggleSettings() {
	...
	if (!components.settings) {
		...
		addElement('p', { class: 'settings-sub-body-header' }, 'What is PasswordManager?', header);
		addElement('img', { class: 'settings-sub-body-header', style: 'margin-left: 4px; height: 26px', src: icons.PassVault.icon }, 'What is PasswordManager?', header);
		addElement('p', { class: 'settings-sub-body', innerHTML: true }, `PasswordManager is an <a href="" onclick="openExternal('github')">open-source program</a> 
		developed by  team init0 that stores your encrypted passwords locally and not on the cloud, prioritizing your privacy.`, aboutSection.body);
		...
		<a href="" onclick="openExternal('instagram')">@init0</a>.
		 If you encounter a bug with this app. 
		 It helps maintain this app, and build future projects.
		 <br>
		 <br>
		 Copyright (C) 2022-present team init0, licensed under GNU GPL v1
		 <br>
...
function openExternal(type) {
	...
	let types = {
		github: 'https://github.com/init0',
		instagram: 'https://www.instagram.com/init0/'
	}
